...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
 ...
 // Calculate days remaining properly (including negative values for expired items)
// Calculate status based on expiration
// Calculate days remaining properly (including negative values for expired items)
// Ensure the expiration date is set and properly formatted
if (empty($errors)) {

    // Debugging: Print the expiration date
    echo "Expires: " . $expires . "<br>";

    // Ensure that $expires is a valid date
    if ($expires) {
        try {
            // Convert $expires to DateTime object
            ...
            // Debugging: Check the interval and days_left
            echo "Interval: " . $interval->format('%r%a') . "<br>";
            $days_left = (int)$interval->format('%r%a');  // %r includes sign

            // Debugging: Check the value of $days_left
            echo "Days Left: " . $days_left . "<br>";

            // Determine status based on days left
            if ($days_left < 0) {
                $status = 'expired';
                echo "Status: expired<br>"; // Debugging status
            } elseif ($days_left <= 1) {
                $status = 'urgent';
                echo "Status: urgent<br>"; // Debugging status
            } elseif ($days_left <= 3) {
                $status = 'warning';
                echo "Status: warning<br>"; // Debugging status
            } else {
                $status = 'normal';
                echo "Status: normal<br>"; // Debugging status
            }

        } catch (Exception $e) {
            // Handle invalid date exception
            echo "Invalid expiration date format!";
            $status = 'normal';  // Default to normal if invalid date
        }
    } else {
        echo "No expiration date provided.<br>";
        $status = 'normal';  // Default to normal if no date is set
    }

} else {
    // If there are errors, set the status to normal (or handle accordingly)
    $status = 'normal';
}

// Debugging: Final status output
echo "Final Status: " . $status . "<br>";

// Debugging: Check final status
